...
**SOS on v3 and v4 shares the user button with stock input handling (2026-09-25, orchestrator).** `TrekLinkSOSGesture` polls `BUTTON_PIN` for a 3-second hold (`TrekLinkSOSGesture.cpp:15`, `:75`), and on T-Beam boards `BUTTON_PIN` is the middle user button (`variants/esp32/tbeam/variant.h:6`, GPIO 38; `variants/esp32s3/tbeam-s3-core/variant.h:9`, GPIO 0). Stock `InputBroker` binds the same pin to its `UserButtonThread` (`InputBroker.cpp:290-306`): a 500 ms hold fires `INPUT_BROKER_SELECT` and a longer hold `INPUT_BROKER_SHUTDOWN`. A 3-second SOS hold therefore passes through SELECT on its way and races the stock long-long-press. The T-Beam documentation assigns the long press to select, not SOS.

**Field reports, root cause not established (leader, 2026-09-25).** These predate the `onboard-queue` work and are observations from hardware tests, not source facts:

| Variant | Observed | Suspected area |
|---|---|---|
| v1 (MPU6050) | Fall detection never triggers | IMU driver or HAL wiring, sample scaling, or the detection thresholds |
| v2 (ICM-20948) | Fall detection did not work when tested; some units stuck in SOS | The same, plus a state that never clears (a race between the fall, button and gesture SOS owners, or an IMU read that stalls) |
| v3 | No fall detection by design (above); button SOS unreliable | The button race above |
| v4 (QMI8658) | Untested; button SOS believed unreliable | The button race above |

Each needs a reproduction on hardware before a fix is designed. They are tracked in the risk register.
