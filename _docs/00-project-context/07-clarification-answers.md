...
| 6 | Incident FSM gaps | Detection confidence (`Confirmed`, `Suspected`) is an attribute, not a state. Dismissing a suspected incident is `Detected` to `Closed` with a mandatory dismissal reason (backlog US-088). Reopen is `Resolved` to `In Progress` only; beacons after `Closed` open a new Incident | Superseded by D-030 |
...
| 27 | Rental FSM | `Created` (agreement generated) → `Active` (checked out) → `Returned` (checked in, invoice issued) → `Closed` (balance settled). `Escalated` from `Active` when a device is not returned within the grace period; `Escalated` → `Closed` only after a loss record | Superseded by D-030 |
